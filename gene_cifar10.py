...
def main():
    global x_ori
    parameters = hyperparameters("cifar10", None)
        ...
        for severity in range(1, 6):#1, 6
            ...
            for imgNo, img in enumerate(x_ori):
                ...
                # if imgNo == 0:
                    ...
                    # plt.close()
                x_new[imgNo] = img_new
            print("{0}-{1}: {2}".format(method_name, severity, x_new.shape))
            np.save(parameters.save_data_root_adv + "{0}-{1}.npy".format(method_name, severity), x_new)
            print("{0}-{1}".format(x_new.min(), x_new.max()))
